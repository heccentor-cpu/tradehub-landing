"use client";

import Image from "next/image";
import Link from "next/link";
import { Search } from "lucide-react";
import LangSwitch from "./LangSwitch";
import type { Dict } from "@/lib/i18n";

function WhatsAppIcon({ className = "w-3.5 h-3.5" }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={className}>
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 0 0-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413Z" />
    </svg>
  );
}

type Props = {
  lang: "es" | "en";
  dict: Dict["header"];
  page?: "home" | "firm";
};

export default function Header({ lang, dict, page = "home" }: Props) {
  const homeHref = `/${lang}`;
  const buildNavHref = (item: { anchor?: string; page?: string }) => {
    if (item.page) return `/${lang}/${item.page}`;
    if (item.anchor) return page === "home" ? `#${item.anchor}` : `/${lang}#${item.anchor}`;
    return "#";
  };
  const ctaHref = page === "home" ? "#agenda" : `/${lang}#agenda`;

  return (
    <header className="bg-white border-b border-bc-border sticky top-0 z-50">
      {/* Utility bar */}
      <div className="hidden md:flex justify-end items-center gap-5 px-8 py-1.5 text-xs text-foreground/70 bg-bc-light">
        <a
          href="https://wa.me/52553407891"
          target="_blank"
          rel="noopener noreferrer"
          className="hover:text-[#25D366] transition-colors flex items-center gap-1.5"
        >
          <WhatsAppIcon className="w-3.5 h-3.5 text-[#25D366]" /> {dict.phone}
        </a>
        <LangSwitch current={lang} />
      </div>

      {/* Main nav */}
      <div className="max-w-[1400px] mx-auto flex items-center justify-between px-6 py-2">
        <div className="flex items-center gap-8">
          <Link href={homeHref} className="relative w-48 h-24 sm:w-56 sm:h-32 flex-shrink-0 block">
            <Image
              src="/logo-v2.png"
              alt="TradeHub Consulting"
              fill
              className="object-contain object-left"
              priority
              quality={100}
            />
          </Link>
          <nav className="hidden lg:flex items-center gap-7 text-[15px] font-medium text-foreground/80">
            {dict.nav.map((item) => (
              <Link
                key={item.label}
                href={buildNavHref(item)}
                className="hover:text-bc-blue transition-colors"
              >
                {item.label}
              </Link>
            ))}
          </nav>
        </div>

        <div className="flex items-center gap-3 md:gap-6">
          {/* LangSwitch visible en móvil; en desktop ya está en la utility bar */}
          <div className="md:hidden">
            <LangSwitch current={lang} />
          </div>
          <button className="text-foreground/70 hover:text-bc-blue hidden md:block" aria-label="Search">
            <Search className="w-5 h-5" />
          </button>
          <Link
            href={ctaHref}
            className="px-4 md:px-6 py-2 md:py-2.5 bg-bc-blue text-white text-xs md:text-sm font-bold rounded hover:bg-bc-dark transition-colors whitespace-nowrap"
          >
            {dict.cta}
          </Link>
        </div>
      </div>
    </header>
  );
}
